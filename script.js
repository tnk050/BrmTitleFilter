...
document.addEventListener("DOMContentLoaded",()=>{
  fullList=document.querySelectorAll('.brm-page li');
...
function createItem (category){
  ...
  if(existingDetail){existingDetail.forEach(item=>selector.removeChild(item))}
  const addList= (()=>{
        const regex = selectSorting(category);
        if(!regex) {return ['All']}
        const dupliceteDetails = fullListArray.map((item)=>regex.exec(item.innerText)[1])
...
function selectSorting(category){
  ...
  const date=/BRM(\d{3,})/;
  const team=/（(\S{2,})）/
  const depart=/km\s([\u30a0-\u30ff\u3040-\u309f\u3005-\u3006\u30e0-\u9fcf]+)/ //[日本語に一致]
  switch(category){
    case 'date':
      return date;
    break;
...
function sortBrm(category){
  // 並べ替え処理
  const target=document.querySelector('.brm-page ul');
...
function sorting(dom,category){
  ...
  if(!sortRegex) return;
  listArray.sort((a,b) =>
  sortRegex.exec(a.innerText)[1].localeCompare(sortRegex.exec(b.innerText)[1] , 'ja', {numeric: true})
  );
...
function filtering(dom,detail){
  ...
  if(existingList){existingList.forEach(item=>dom.removeChild(item))}
  const filterList = (detail==='All')?fullListArray:fullListArray.filter(item=>item.innerText.match(detail));
